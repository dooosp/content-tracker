...
const naverClient = {
  ...
  async fetchAll() {
    const missing = [];
    if (!NAVER.clientId) missing.push('NAVER_CLIENT_ID missing');
    if (!NAVER.clientSecret) missing.push('NAVER_CLIENT_SECRET missing');
    if (missing.length > 0) {
      return {
        posts: [],
    ...
    // 중복 URL 제거
    const unique = dedupeBy(allPosts, (post) => post.url);
