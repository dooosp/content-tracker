...
const contentAnalyzer = {
  ...
  calcViewVelocity(post) {
    ...
    if (post.source === 'naver') {
      ...
      return Math.round(post.viewCount / days);
    }

    // Reddit: upvotes per hour
    return Math.round(post.viewCount / hours);
  },
  ...
  calcEngagementRate(post) {
    ...
    return (post.likeCount + post.commentCount) / post.viewCount;
  },
  ...
  detectCross(snapshots) {
    ...
    const views = snapshots.map(s => s.viewCount);
    ...
    const prevViews = views.slice(0, -1);
