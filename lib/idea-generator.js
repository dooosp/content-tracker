...
function getLLM() {
  if (!createLLMClient) {
    try {
      ({ createLLMClient } = require('llm-client'));
    } catch (error) {
      const wrapped = new Error(`llm-client unavailable: ${toErrorMessage(error)}`);
      wrapped.code = 'LLM_CLIENT_UNAVAILABLE';
      throw wrapped;
    }
  }
...
function buildPortfolioSummary(portfolio = {}) {
  if (Array.isArray(portfolio)) {
    return portfolio
      .map((item) => `${item.topic}: ${item.videoCount || 0}posts (${item.share || 0}%)`)
      .join(', ');
  }
...
function buildIdeaPrompt(topPosts, portfolioSummary) {
  return `You are a content strategist analyzing trending posts.

## Top 20 Posts (ranked by engagement score)
${topPosts.map(post => `${post.rank}. [${post.source}] ${post.title} (score: ${post.score}, topic: ${post.topic})`).join('\n')}
...
export async function generateIdeas(topPosts = [], portfolio = {}, fetchedAt = 'na') {
  ...
  const prompt = buildIdeaPrompt(top20, portfolioSummary);
