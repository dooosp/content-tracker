...
const contentScorer = {
  ...
  scoreAll(posts, context = {}) {
    const postsWithTopic = posts.map((post) => ({
      ...post,
      topic: post.topic || this.detectTopic(post.title),
    }));

    const channelAvgBySource = this.calcSourceAverages(postsWithTopic);
    const topicAvgBySource = this.calcTopicAveragesBySource(postsWithTopic);

    return postsWithTopic
      .map((post) => ({
        ...post,
        scoring: this.score(post, {
          channelAvg: channelAvgBySource[post.source] || 0,
          topicAvg: topicAvgBySource[post.source] || {},
          snapshots: context.snapshotMap?.[post.postId] || [],
        }),
      }))
  ...
  calcGrowthScore(post, snapshots = []) {
    ...
    if (trend.shortMA !== null && trend.longMA !== null && trend.shortMA > trend.longMA) return 0.7;
  ...
  calcTopicScore(post, topicAvg = {}) {
    const topic = post.topic || this.detectTopic(post.title);
    const avg = topicAvg[topic];
    if (!avg || avg <= 0) return 0.5;

    const ratio = safeDivide(post.viewCount, avg, 0);
  ...
  calcChannelRelative(post, channelAvg = 0) {
    ...
    const ratio = safeDivide(post.viewCount, channelAvg, 0);
  ...
  detectTopic(title) {
    ...
    for (const [topic, keywords] of Object.entries(TOPIC_KEYWORDS)) {
      for (const keyword of keywords) {
        if (upper.includes(keyword.toUpperCase())) return topic;
      }
    }
    return 'Other';
  },
  ...
  calcSourceAverages(posts) {
    ...
    for (const post of posts) {
      ...
      groups[source].push(Number(post.viewCount || 0));
    }

    const averages = {};
    for (const [source, views] of Object.entries(groups)) {
      const sum = views.reduce((acc, value) => acc + value, 0);
      averages[source] = safeDivide(sum, views.length, 0);
    }
    return averages;
  },
  ...
  calcTopicAveragesBySource(posts) {
    ...
    for (const post of posts) {
      ...
      const topic = post.topic || this.detectTopic(post.title);
      if (!groups[source]) groups[source] = {};
      if (!groups[source][topic]) groups[source][topic] = [];
      groups[source][topic].push(Number(post.viewCount || 0));
    }

    const averages = {};
    for (const [source, topics] of Object.entries(groups)) {
      averages[source] = {};
      for (const [topic, views] of Object.entries(topics)) {
        const sum = views.reduce((acc, value) => acc + value, 0);
        averages[source][topic] = safeDivide(sum, views.length, 0);
      }
    }
    return averages;
  },
